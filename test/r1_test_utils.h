...
struct ManagerTestAccess
{
    ...
    static std::shared_ptr<ControlSignalSource<msgT, srvT>>
    createSource(rclcpp::Node* node, const ControlSignalInfo& info, int64_t rateWindowNs = 1'000'000'000)
    {
    ...
    template <typename msgT, typename srvT = void>
    static std::shared_ptr<ControlSignalSink<msgT, srvT>>
    createSink(rclcpp::Node* node, const ControlSignalInfo& info, int64_t rateWindowNs = 1'000'000'000)
    {
        auto sink =
            std::shared_ptr<ControlSignalSink<msgT, srvT>>(new ControlSignalSink<msgT, srvT>(node, info, rateWindowNs));
        sink->_bindTransport();
        return sink;
    }

    template <typename Entity> static EntityDecision calc(const Entity& e, int64_t nowNs)
    {
        return e._calcStatus(nowNs);
    }

    template <typename Entity> static void apply(Entity& e, const EntityDecision& d) { e._applyStatus(d); }

    template <typename Entity> static bool trySealLocalTerminal(Entity& e, const EntityDecision& d)
    {
    ...
    template <typename SourceT> static void recordOutcomeFailure(SourceT& s, uint64_t seq, int64_t nowNs)
    {
        s._recordOutcomeFailure(seq, nowNs);
    }

    template <typename SourceT> static void recordOutcomeSuccess(SourceT& s, uint64_t seq)
    {
    ...
    template <typename Entity> static std::optional<EntityDecision> tick(Entity& e, int64_t nowNs)
    {
...
protected:
    void SetUp() override
    {
        node_ = std::make_shared<rclcpp::Node>("r1_test_" + std::to_string(reinterpret_cast<uintptr_t>(this) % 100000));
