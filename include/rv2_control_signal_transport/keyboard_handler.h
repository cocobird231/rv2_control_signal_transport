...
public:
        ...
        ownsFd_(false)
    {
        ...
        if (fd_ < 0)
        {
            ...
            const int efd = tryAutoDetect ? _autoDetect() : ::open(devicePath.c_str(), O_RDONLY);
            if (efd >= 0)
            {
                mode_ = InputMode::EVDEV;
                fd_ = efd;
        ...
        if (fd_ < 0)
        {
            throw std::runtime_error(std::string("KeyboardHandler: cannot open input source") +
...
private:
    ...
    void _runStdin()
    {
        ...
        while (running_.load(std::memory_order_relaxed))
        {
            ...
            {
                fd_, POLLIN, 0
            };  // fd_ is /dev/tty or STDIN_FILENO
    ...
    static std::size_t _idx(Key k) noexcept { return static_cast<std::size_t>(k); }
    ...
    static bool _testBit(int bit, const uint8_t* arr) noexcept { return (arr[bit / 8] >> (bit % 8)) & 1u; }
    ...
    static int _autoDetect()
    {
        ...
        while (result < 0 && (ent = ::readdir(d)) != nullptr)
        {
            ...
            if (::ioctl(fd, EVIOCGBIT(0, sizeof(evBits)), evBits) >= 0 && _testBit(EV_KEY, evBits))
            {
                // Must have WASD keys — distinguishes keyboards from mice/joysticks.
                uint8_t keyBits[(KEY_MAX + 7) / 8] = {};
                if (::ioctl(fd, EVIOCGBIT(EV_KEY, sizeof(keyBits)), keyBits) >= 0 && _testBit(KEY_W, keyBits) &&
                    _testBit(KEY_A, keyBits))
                {
                    ::close(fd);
