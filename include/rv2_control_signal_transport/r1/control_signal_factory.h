...
public:
    ...
    bool Register(const std::string& name)
    {
        ...
        e.sourceCreator = [](rclcpp::Node* node, const InfoT& info,
        ...
        e.sinkCreator = [](rclcpp::Node* node, const InfoT& info,
                           int64_t rateWindowNs)
    ...
    std::shared_ptr<BaseControlSignalSource> CreateSource(
        const std::string& type, rclcpp::Node* node, const InfoT& info,
        std::string* errOut = nullptr,
        int64_t rateWindowNs = 1'000'000'000) noexcept
    {
        return _create<BaseControlSignalSource>(type, node, info, errOut,
                                                rateWindowNs, /*source=*/true);
    }

    std::shared_ptr<BaseControlSignalSink> CreateSink(
        const std::string& type, rclcpp::Node* node, const InfoT& info,
        std::string* errOut = nullptr,
        int64_t rateWindowNs = 1'000'000'000) noexcept
    {
        return _create<BaseControlSignalSink>(type, node, info, errOut,
                                              rateWindowNs, /*source=*/false);
    }
...
private:
    ...
    struct Entry
    {
        ...
        std::function<std::shared_ptr<BaseControlSignalSource>(
            rclcpp::Node*, const InfoT&, int64_t)> sourceCreator;
        std::function<std::shared_ptr<BaseControlSignalSink>(
            rclcpp::Node*, const InfoT&, int64_t)> sinkCreator;
    };

    template<typename BaseT>
    std::shared_ptr<BaseT> _create(const std::string& type, rclcpp::Node* node,
                                   const InfoT& info, std::string* errOut,
                                   int64_t rateWindowNs, bool source) noexcept
    {
        try
        {
            Entry entry;
            ...
            {
                if (errOut)
                    *errOut = "type '" + type + "' is topic-only (no service type "
                              "registered, §7.2)";
                return nullptr;
            }
...
#define R1_REGISTER_CONTROL_SIGNAL(UniqueId, name_str, MsgType, SrvType)      \
    ...
    const bool r1_registered_##UniqueId =                                     \
        ::rv2_interfaces::r1::ControlSignalFactory::Instance()                \
