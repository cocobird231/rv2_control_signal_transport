...
struct RetryPolicy
{
    ...
    RetryPolicy(int64_t initialDelayMs_,
                int64_t maxDelayMs_,
                double jitterRatio_,
                uint32_t maxInitialAttempts_,
    ...
    bool valid() const
    {
        return initialDelayMs > 0 && maxDelayMs >= initialDelayMs && jitterRatio >= 0.0 && jitterRatio <= 1.0 &&
               maxInFlight >= 1;
    }
};
...
{
    explicit ManagerOptions(RetryPolicy policy) :
    ...
    std::string validate() const
    {
        if (statusIntervalMs <= 0)
            return "statusIntervalMs must be > 0";
        if (pendingTtlMs <= 0)
            return "pendingTtlMs must be > 0";
        if (maxRegisterTimeoutMs <= 0)
            return "maxRegisterTimeoutMs must be > 0";
        if (rateWindowNs <= 0)
            return "rateWindowNs must be > 0";
        if (masterName.empty())
            return "masterName must not be empty";
        if (!retryPolicy.valid())
            return "retryPolicy invalid";
        if (csmTimeoutNs < 0 || csmDisconnectTimeoutNs < 0)
            return "CSM thresholds must be >= 0";
        if (csmTimeoutNs > 0 && csmDisconnectTimeoutNs > 0 && csmDisconnectTimeoutNs <= csmTimeoutNs)
...
public:
    ...
    ControlSignalManager(rclcpp::Node* node, const std::string& name, const ManagerOptions& opt) :
        node_(node),
        name_(name),
        opt_(opt),
        csmInstanceId_(detail::makeUuid())
    {
        ...
        mgmtGroup_ = node_->create_callback_group(rclcpp::CallbackGroupType::Reentrant);
        tickGroup_ = node_->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
        ...
        infoReqSrv_ = node_->create_service<InfoReqSrv>(
            ...
            [this, life = life_](const std::shared_ptr<InfoReqSrv::Request>, std::shared_ptr<InfoReqSrv::Response> res)
            {
                if (!life->alive.load())
                    return;
                life->active.fetch_add(1);
        ...
        statusPub_ = node_->create_publisher<ManagerStatusT>(name_ + "/status", rclcpp::QoS(10));
        masterRegClient_ =
            node_->create_client<CsmRegisterSrv>(opt_.masterName + "/register", rclcpp::ServicesQoS(), mgmtGroup_);
        heartbeatClient_ =
            node_->create_client<CsmHeartbeatSrv>(opt_.masterName + "/heartbeat", rclcpp::ServicesQoS(), mgmtGroup_);
    ...
    RegisterResult registerSource(const InfoT& info, int64_t timeoutMs = 5000)
    {
        ...
        if (!factory.has(info.type) || (info.mode == InfoT::MODE_SERVICE && !factory.serviceCapable(info.type)))
        ...
        std::shared_ptr<SourceRegistrationSlot> slot;
        {
            ...
            for (const auto& [c, s] : sources_)
                if (c == info.controller_name || s->info.channel_name == info.channel_name)
        ...
        if (!client->service_is_ready())
            return _initialFailure(slot,
                                   identity,
                                   RegisterError::TARGET_UNREACHABLE,
                                   RetryReason::INITIAL_UNREACHABLE,
                                   "target manage service not ready",
                                   handle);
        ...
        if (future.wait_for(std::chrono::milliseconds(timeoutMs)) != std::future_status::ready)
        {
            ...
            _sendBestEffortUnregister(info.target_manager_name, info.controller_name, identity);
        ...
        case ManageSrv::Response::RESPONSE_RETRYABLE_CONFLICT:
            return _initialFailure(slot,
                                   identity,
                                   RegisterError::RETRYABLE_CONFLICT,
        ...
        std::string err;
        auto endpoint = factory.CreateSource(info.type, node_, info, &err, opt_.rateWindowNs);
        if (!endpoint)
        {
            _sendBestEffortUnregister(info.target_manager_name, info.controller_name, identity);
            _dropSlot(slot, info.controller_name);
    ...
    template <typename msgT> bool registerCallback(std::function<void(const msgT&, const InfoT&)> cb)
    {
        const std::string type = ControlSignalFactory::Instance().typeKey(std::type_index(typeid(msgT)));
        if (type.empty())
            return false;
...
private:
    ...
    rclcpp::Client<ManageSrv>::SharedPtr _manageClient(const std::string& target)
    {
        ...
        if (!c)
            c = node_->create_client<ManageSrv>(target + "/control_signal_manage", rclcpp::ServicesQoS(), mgmtGroup_);
        return c;
    }

    void _dropSlot(const std::shared_ptr<SourceRegistrationSlot>& slot, const std::string& controller)
    {
        {
            std::unique_lock<std::shared_mutex> sl(slot->slotMtx);
            slot->desired = false;
                                   ...
                                   SourceHandle handle)
    {
        ...
        if (d7Gated && (!opt_.retryPolicy.autoRetryInitial || opt_.retryPolicy.maxInitialAttempts == 0))
        {
                       ...
                       bool everSucceeded)
    {
        ...
        const bool quarantined = neverActive >= opt_.retryPolicy.quarantineThreshold;
        const int64_t delayMs = quarantined ? opt_.retryPolicy.maxDelayMs : opt_.retryPolicy.initialDelayMs;
        {
            std::lock_guard<std::mutex> lk(retryMtx_);
        ...
        if (quarantined)
            _notify(EventKind::SUSPECTED_DATA_PATH_FAULT,
                    controller,
                    RegisterError::OK,
                    generation,
                                   ...
                                   const RegistrationIdentity& identity)
    {
        ...
        client->async_send_request(req,
                                   [](rclcpp::Client<ManageSrv>::SharedFuture)
                                   {
                                   });  // best-effort
    }

    void _notify(EventKind kind,
                 const std::string& controller,
                 RegisterError result,
                 uint64_t generation,
    ...
    void _installSourceStateCb(const std::shared_ptr<BaseControlSignalSource>& ep,
                               ...
                               const StateCb& cb);
    void
    _installSinkStateCb(const std::shared_ptr<BaseControlSignalSink>& ep, ControlSignalState state, const StateCb& cb);
    ...
    void _applyTypedCb(const std::string& type, BaseControlSignalSink::ErasedMsgCb cb)
    {
    ...
    void _onManageRegister(const ManageSrv::Request& req, ManageSrv::Response& res)
    {
        ...
        if (req.source_manager_name.empty() || req.source_csm_instance_id.empty() || req.registration_id.empty() ||
            req.attempt_generation == 0)
        {
        ...
        {
            res.response = ManageSrv::Response::RESPONSE_PERMANENT_REJECTION;
            res.reason = "mis-routed: target '" + req.info.target_manager_name + "' is not '" + name_ + "'";
            return;
        }
        ...
        if (!factory.has(req.info.type) ||
            (req.info.mode == InfoT::MODE_SERVICE && !factory.serviceCapable(req.info.type)))
        {
            res.response = ManageSrv::Response::RESPONSE_PERMANENT_REJECTION;
        ...
        const RegistrationIdentity identity{req.source_csm_instance_id, req.registration_id, req.attempt_generation};
        // Duplicate check + idempotency + PENDING emplace under one lock.
        {
            ...
            if (it != sinks_.end())
            {
                if (it->second.identity == identity)
                {
                    ...
                    res.response = it->second.phase == RegistrationPhase::REGISTERED
        ...
        std::string err;
        auto endpoint = factory.CreateSink(req.info.type, node_, req.info, &err, opt_.rateWindowNs);
        if (!endpoint)
        {
            std::unique_lock<std::shared_mutex> lk(sinkMtx_);
            const auto it = sinks_.find(req.info.controller_name);
            ...
            if (it != sinks_.end() && it->second.identity == identity &&
                it->second.phase == RegistrationPhase::PENDING && !it->second.endpoint)
                sinks_.erase(it);
            res.response = ManageSrv::Response::RESPONSE_PERMANENT_REJECTION;
        ...
        // TTL reaper and matching UNREGISTERs race through this same lock.
        {
            ...
            const auto it = sinks_.find(req.info.controller_name);
            if (it == sinks_.end() || it->second.identity != identity ||
                it->second.phase != RegistrationPhase::PENDING || it->second.pendingRemoval.has_value())
            {
                lk.unlock();
                endpoint->shutdown();
    ...
    void _onManageUnregister(const ManageSrv::Request& req, ManageSrv::Response& res)
    {
        ...
        if (it == sinks_.end())
        {
            ...
            if (done != completedRemovals_.end() && done->second == identity.attemptGeneration)
            {
    ...
    void _onGetNotifications(const CsmNotifySrv::Request& req, CsmNotifySrv::Response& res)
    {
        ...
        case CsmNotifySrv::Request::KIND_STATE:
            ...
            _notify(EventKind::PEER_STATE, "", RegisterError::OK, 0, "", req.entries);
            break;
        case CsmNotifySrv::Request::KIND_CSM_TIMEOUT:
        {
            const bool healthy = req.peer_csm_health == CsmNotifySrv::Request::PEER_HEALTH_ACTIVE;
            anyStale = !_updatePeerHealth(req.entries, healthy ? PeerHealth::ACTIVE : PeerHealth::TIMEOUT);
            if (!anyStale)
                _notify(healthy ? EventKind::PEER_CSM_ACTIVE : EventKind::PEER_CSM_TIMEOUT,
                        "",
                        RegisterError::OK,
                        0,
                        "",
        ...
        {
            const RemovalReason reason = req.kind == CsmNotifySrv::Request::KIND_DISCONNECTED
    ...
    bool _updatePeerHealth(const std::vector<EntryStatusT>& entries, PeerHealth ph)
    {
        ...
        for (const auto& e : entries)
        {
            ...
            {
                std::unique_lock<std::shared_mutex> sl(slot->slotMtx);
                if (slot->registrationId == e.registration_id && slot->attemptGeneration == e.attempt_generation)
                {
                    slot->peerHealth = ph;
                    matched = true;
                }
            ...
            const RegistrationIdentity identity{e.source_csm_instance_id, e.registration_id, e.attempt_generation};
            std::unique_lock<std::shared_mutex> lk(sinkMtx_);
    ...
    bool _queueMatchingRemovals(const std::vector<EntryStatusT>& entries, RemovalReason reason)
    {
        bool matched = false;
        for (const auto& e : entries)
        {
    ...
    void _tickCalcAndCommit(int64_t now)
    {
                ...
                w.slot = s;
                {
                    ...
                    if (s->phase != RegistrationPhase::REGISTERED || !s->endpoint)
                    {
                        if (s->pendingRemoval || !s->desired)
                        {
                            ...
                            w.identity = {csmInstanceId_, s->registrationId, s->attemptGeneration};
        ...
        for (auto& w : sw)
        {
            if (!w.endpoint || w.pendingRemoval || w.decision.status.state == ControlSignalState::DISCONNECTED)
                continue;
            bool ok = false;
            {
                std::unique_lock<std::shared_mutex> sl(w.slot->slotMtx);
                if (w.slot->desired && w.slot->phase == RegistrationPhase::REGISTERED &&
                    w.slot->endpoint == w.endpoint && w.slot->attemptGeneration == w.identity.attemptGeneration)
                {
        ...
        for (auto& w : kw)
        {
            if (w.pendingRemoval || w.decision.status.state == ControlSignalState::DISCONNECTED)
                continue;
            bool ok = false;
            {
                std::unique_lock<std::shared_mutex> lk(sinkMtx_);
                const auto it = sinks_.find(w.controller);
                if (it != sinks_.end() && it->second.identity == w.identity && it->second.endpoint == w.endpoint &&
                    !it->second.pendingRemoval)
                {
        ...
        for (auto& w : sw)
        {
            const bool localTerminal =
                w.endpoint && w.decision.status.state == ControlSignalState::DISCONNECTED && !w.pendingRemoval;
        ...
        for (auto& w : kw)
        {
            const bool localTerminal = w.decision.status.state == ControlSignalState::DISCONNECTED && !w.pendingRemoval;
            if (!localTerminal && !w.pendingRemoval)
                continue;
    ...
    void _terminalCommitSource(SourceWork& w, int64_t now, bool localTerminal)
    {
        ...
        if (localTerminal)
        {
            reason = w.decision.cause == LivenessCause::RESPONSE_FAILURE ? RemovalReason::RESPONSE_FAILURE
        ...
        const bool keepIntent = reason == RemovalReason::RESPONSE_FAILURE ||
                                reason == RemovalReason::PEER_DISCONNECTED || reason == RemovalReason::PAIR_MISSING;
        bool neverActive = false;
        {
            std::unique_lock<std::shared_mutex> sl(w.slot->slotMtx);
            if (w.slot->endpoint != w.endpoint || w.slot->attemptGeneration != w.identity.attemptGeneration)
                return;  // a newer generation took over: leave it alone
            w.slot->lastStatus = {ControlSignalState::DISCONNECTED, w.decision.status.rateHz};
        ...
        bool retainSlot;
        {
            ...
            w.slot->phase = retainSlot ? RegistrationPhase::RETRY_WAIT : RegistrationPhase::REMOVING;
        ...
        {
            _enqueueRetry(w.slot,
                          w.identity.registrationId,
                          reason == RemovalReason::RESPONSE_FAILURE
                              ? RetryReason::RESPONSE_FAILURE
                              : (reason == RemovalReason::PAIR_MISSING ? RetryReason::PAIR_MISSING
        ...
        _sendBestEffortUnregister(w.slot->info.target_manager_name, w.controller, w.identity);

        const EventKind kind = reason == RemovalReason::PEER_DISCONNECTED ? EventKind::PEER_DISCONNECTED
                               : reason == RemovalReason::PAIR_MISSING    ? EventKind::PAIR_MISSING
                                                                          : EventKind::LOCAL_DISCONNECTED;
        _notify(kind, w.controller, RegisterError::OK, w.identity.attemptGeneration, "terminal removal");
    }
    ...
    void _terminalCommitSink(SinkWork& w, bool localTerminal)
    {
        ...
        }
        {
            ...
            if (it == sinks_.end() || it->second.identity != w.identity || it->second.endpoint != w.endpoint)
                return;
            it->second.lastStatus = {ControlSignalState::DISCONNECTED, w.decision.status.rateHz};
        ...
        w.endpoint->shutdown();
        {
            ...
            {
                completedRemovals_[w.identity.registrationId] = w.identity.attemptGeneration;
                if (completedRemovals_.size() > 4096)
                    completedRemovals_.erase(completedRemovals_.begin());
                sinks_.erase(it);
            }
        }
        _notify(EventKind::LOCAL_DISCONNECTED,
                w.controller,
                RegisterError::OK,
                w.identity.attemptGeneration,
                "sink terminal removal");
    }
    ...
    void _tickPublishAndHeartbeat(int64_t now)
    {
        ...
        if (!masterRegistered_.load() && masterRegClient_->service_is_ready())
        {
            ...
            if (!regInFlight_)
            {
                ...
                req->registration_grace_ns = (opt_.maxRegisterTimeoutMs + 2 * opt_.statusIntervalMs) * 1'000'000;
                masterRegClient_->async_send_request(
                    req,
        ...
        if (masterRegistered_.load())
        {
            ...
            if (!heartbeatInFlight_)
            {
                ...
                req->csm_name = name_;
                req->csm_instance_id = csmInstanceId_;
                heartbeatClient_->async_send_request(
                    req,
    ...
    void _tickRetryAndMaintenance(int64_t now)
    {
        ...
        uint32_t inFlightCount = 0;
        {
            ...
            {
                auto& pr = it->second;
                if (pr.inFlight || now < pr.nextTryNs || inFlightCount + due.size() >= opt_.retryPolicy.maxInFlight)
                {
                    ++it;
                    continue;
                }
                ...
                if (!pr.everSucceeded && pr.reason == RetryReason::INITIAL_UNREACHABLE &&
                ...
                pr.inFlight = true;
                pr.inFlightDeadlineNs = now + opt_.maxRegisterTimeoutMs * 1'000'000;
                ++pr.attempts;
        ...
        for (auto& [rid, slot] : exhausted)
        {
            ...
            _dropSlot(slot, slot->info.controller_name);
            _notify(EventKind::RETRY_FAILED,
                    slot->info.controller_name,
                    RegisterError::TARGET_UNREACHABLE,
                    gen,
    ...
    void _applyRetryCompletion(const RetryCompletion& c, int64_t now)
    {
        ...
        if (c.result == RegisterError::OK)
        {
            ...
            InfoT info;
            {
                ...
                if (!slot->desired || slot->attemptGeneration != c.identity.attemptGeneration ||
                    slot->registrationId != c.identity.registrationId)
                    return;
                info = slot->info;
            }
            endpoint = ControlSignalFactory::Instance().CreateSource(info.type, node_, info, &err, opt_.rateWindowNs);
            if (!endpoint)
                return;
            ...
            _notify(EventKind::RETRY_SUCCEEDED,
                    slot->info.controller_name,
                    RegisterError::OK,
                    c.identity.attemptGeneration,
                    c.reason);
            return;
        }
        ...
        if (permanent)
        {
            ...
            _dropSlot(slot, slot->info.controller_name);
            _notify(
                EventKind::RETRY_FAILED, slot->info.controller_name, c.result, c.identity.attemptGeneration, c.reason);
            return;
        }
        ...
        }
        {
            ...
            std::uniform_real_distribution<double> dist(1.0 - opt_.retryPolicy.jitterRatio,
                                                        1.0 + opt_.retryPolicy.jitterRatio);
            it->second.nextTryNs = now + static_cast<int64_t>(delayMs * dist(jrng)) * 1'000'000;
        }
        (void)everSucceeded;
        _notify(EventKind::RETRY_FAILED, slot->info.controller_name, c.result, c.identity.attemptGeneration, c.reason);
    }
                             ...
                             int64_t now)
    {
        ...
        req->info = info;
        auto client = _manageClient(info.target_manager_name);
        _notify(EventKind::RETRY_STARTED, info.controller_name, RegisterError::OK, identity.attemptGeneration, "");
        if (!client->service_is_ready())
        {
            std::lock_guard<std::mutex> lk(retryMtx_);
            RetryCompletion c{identity, RegisterError::TARGET_UNREACHABLE, "manage service not ready"};
...
inline void ControlSignalManager::_installSourceStateCb(const std::shared_ptr<BaseControlSignalSource>& ep,
                                                        ControlSignalState state,
                                                        const StateCb& cb)
{
...
inline void ControlSignalManager::_installSinkStateCb(const std::shared_ptr<BaseControlSignalSink>& ep,
                                                      ControlSignalState state,
                                                      const StateCb& cb)
{
    ep->_setStateCallbackErased(state, cb);
...
inline void ControlSignalManager::_installAllStateCbs(const std::shared_ptr<BaseControlSignalSource>& ep)
{
    std::lock_guard<std::mutex> lk(stateCbMtx_);
    for (size_t i = 0; i < sourceStateCbs_.size(); ++i)
        if (sourceStateCbs_[i])
            ep->_setStateCallbackErased(static_cast<ControlSignalState>(i), sourceStateCbs_[i]);
}

inline void ControlSignalManager::_installAllStateCbs(const std::shared_ptr<BaseControlSignalSink>& ep)
{
    std::lock_guard<std::mutex> lk(stateCbMtx_);
    for (size_t i = 0; i < sinkStateCbs_.size(); ++i)
        if (sinkStateCbs_[i])
            ep->_setStateCallbackErased(static_cast<ControlSignalState>(i), sinkStateCbs_[i]);
}
